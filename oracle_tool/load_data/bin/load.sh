...
echo "###################################################################"
echo "#                                                                 #"
echo "#             load.sh:load txt file to oracle database            #"
echo "#             Usage:.load.sh ../xx/xx/configrue.cfg               #"
echo "#             Delimiter is | and end with |                       #"
echo "#                                                                 #"
echo "###################################################################"
cd $HOME/mig/bin/script
workdir=`pwd`
refdir=$workdir
tmpfile="${refdir}/temp"
echo 'DATA_PATH:'$tmpfile
logdir="${refdir}/../../log"
logfile="${refdir}/../../log/load.log"
env_cfgfile=$1
echo 'env_cfgfile is:'$env_cfgfile
...
datadir=`awk '/^\[datapath\]/{getline;print $1;exit}' ${env_cfgfile}`
